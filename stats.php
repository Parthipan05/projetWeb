...
<section>
    ...
    // Colonne Mai 2026 depuis l'API (temps réel)
    $moyennes = calculer_moyennes_nationales();
    ?>
            ...
            <tbody>
				...
				foreach ($carburants as $nom => $cle) {
					...
					foreach ($historique as $mois => $prix) {
						$couleur = $prix[$cle] < 1.80 ? '#d4edda' : ($prix[$cle] > 2.00 ? '#f8d7da' : '#fff3cd');
						echo "<td class='cellule-prix' style='text-align:center; background-color:" . $couleur . ";'>";
						echo number_format($prix[$cle], 2, ',', '') . " €";
						echo "</td>";
					}
					if ($moyennes[$cle] !== null) {
						$couleur = $moyennes[$cle] < 1.80 ? '#d4edda' : ($moyennes[$cle] > 2.00 ? '#f8d7da' : '#fff3cd');
						echo "<td class='cellule-prix' style='text-align:center; background-color:" . $couleur . "; font-weight:bold;'>";
    ...
    <div class="legende-tendances">
        <span class="legende-item" style="background-color:#d4edda;">&#9632; Prix bas</span>
        <span class="legende-item" style="background-color:#fff3cd;">&#9632; Prix moyen</span>
        <span class="legende-item" style="background-color:#f8d7da;">&#9632; Prix élevé</span>
    </div>
